...
class Ball:
    def __init__(self):
        self.size = 24
        self.base_speed = 4.0 # Increased from 3.0
        self.reset()
...
def next_level():
    ...
    level_complete = False
    level_complete_timer = 0
    ball.base_speed = min(9, 4.0 + (level - 1) * 1.0)  # 4.0 -> 5.0 -> 6.0 etc
    init_bricks()
    ball.reset()
...
def draw():
    ...
    ctx.fillText(f"TOP: {display_high}", 300, 28)
    
    ctx.fillText(f"LEVEL {level}", 500, 28)
