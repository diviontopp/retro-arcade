...
ASSET_BASE = "/assets/spaceinvaders/"
...
class Projectile:
    def __init__(self, x, y, type_id, target_x=None, target_y=None):
        ...
        elif type_id == 'enemy_laser': self.dy = 6; self.w = 4; self.h = 16; self.color = '#FF0000'
...
class GameState:
    ...
    def next_level(self):
        ...
        self.enemy_speed = min(3.5, 1.0 + ((self.level - 1) * 0.1))
...
def update_game_logic():
    ...
    if not game.boss_active and game.level > 1 and diving_count < 2 and random.random() < 0.003: 
        if forming:
    ...
    for e in active:
        ...
        if e.type_id != 99 and random.random() < 0.0005: game.projectiles.append(Projectile(e.x+24, e.y+48, 'enemy_laser'))
            ...
            if check_collision(br, player_rect):
                ...
                if game.invincibility_timer == 0:
                    ...
                    if game.player_health <= 0:
                        game.state = GameStateEnum.GAME_OVER; js.window.setGameOver(True); js.window.triggerSFX('game_over')
                continue
...
def loop(t):
    ...
    if game.state == GameStateEnum.PLAYING:
        ...
        if game.invincibility_timer % 10 < 5:
             if player_img.complete: ctx.drawImage(player_img, game.playerX, game.playerY, 64, 64)
    ...
    if game.state == GameStateEnum.GAME_OVER:
        ...
        ctx.fillStyle = "white"; ctx.font = "30px Monospace"; ctx.shadowBlur = 0
        ctx.fillText("Press ENTER to Restart", SCREEN_WIDTH//2, SCREEN_HEIGHT//2 + 50)
